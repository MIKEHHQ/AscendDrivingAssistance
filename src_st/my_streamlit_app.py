...
from yolov4_st import detection
...
def main():
	...
	f = open('instructions.md', 'r')
	fileString = f.read()
	readme_text = st.markdown(fileString)
...
def run_the_app():
	...
	if st.session_state.first_time:
		...
		if (cap.is_opened()):
			for i in range(61):
				my_bar.progress(i)
				frame = cap.read() # (720, 1280, 3) mp4文件读进来是RGB
				ret, result = detection(st.session_state.model, frame) # What type?
				frame = cv2.resize(ret, dsize=(960, 540))
				save_frame(frame)
				st.session_state.road_image.image(frame, caption=None, width=None, use_column_width=True, clamp=True, channels="RGB", output_format="auto")
	...
	if (st.session_state.cap.is_opened()):
		while(True):
			if not st.session_state.stop:
				frame = st.session_state.cap.read() # (720, 1280, 3) mp4文件读进来是RGB
				ret, result = detection(st.session_state.model, frame) # What type?
				frame = cv2.resize(ret, dsize=(960, 540))
				save_frame(frame)
				st.session_state.road_image.image(frame, caption=None, width=None, use_column_width=True, clamp=False, channels="RGB", output_format="auto")
				st.session_state.result_return.write(result)
			else:
