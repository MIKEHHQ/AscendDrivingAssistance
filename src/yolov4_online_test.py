...
def main():
    ...
    while (cap.isOpened()):
        ...
        if ret == True:
            ...
            success, img = cv.imencode(".jpg", frame)

            chan.send_image(400, 300, img)
            time3 = time.time()
            print("Successfully send an img!", time1-time0, time2-time1, time3-time2)
            # chan.send_detection_data(MODEL_WIDTH, MODEL_HEIGHT, frame_with_lane, [])
        # Break the loop
        else:
            break
    cap.release()
