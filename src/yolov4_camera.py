...
from atlas_utils.camera import Camera # Add camera
from atlas_utils.acl_dvpp import Dvpp # Add Dvpp to process image
...
labels = ["person",
        ...
        "scissors", "teddy bear", "hair drier", "toothbrush"] # Remain to be cut
INPUT_DIR = '../data/'
OUTPUT_DIR = '../out/'
...
acl_resource.init()
dvpp = Dvpp(acl_resource) # Add dvpp to process image
...
def preprocess(frame):
    ...
    image = Image.fromarray(cv.cvtColor(frame, cv.COLOR_BGR2RGB)) # numpy BGR to RGB image
    img_h = image.size[1]
    img_w = image.size[0]
    net_h = MODEL_HEIGHT
    net_w = MODEL_WIDTH
...
def my_preprocess(frame):
    image = frame.nparray().reshape(-1, frame.width).astype(np.uint8) # AclImage (yuv420sp) to numpy
    np_image = cv.cvtColor(image, cv.COLOR_YUV420sp2RGB) # RGB image
    ret = np_image.copy()
...
def main():
    ...
    while (cap.is_opened()):
        ...
        if frame:
            # preprocess
            time0 = time.time()
            data, orig, ret  = my_preprocess(frame)
            ...
            # Process lane line
            frame_with_lane = ret
            frame_with_lane = preprocess_frame(ret)
            ...
            success, img = cv.imencode(".jpg", frame_with_lane)
            i += 1
            path = "../out/images2/"+str(i)+".jpg"
            cv.imwrite(path, frame_with_lane)
            chan.send_image(1280, 720, img)
            time1 = time.time()
            print("Successfully send an img!", time1-time0)
        # Break the loop
        else:
            break
    print("Execute end")
...
if __name__ == '__main__':
    ...
    lf = LaneFinder.LaneFinder(ORIGINAL_SIZE, WARPED_SIZE, cam_matrix, dist_coeffs, perspective_transform, pixels_per_meter)
    main()
    gc.collect()
